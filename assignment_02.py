...
Original file is located at
    https://colab.research.google.com/drive/1LMZdiGQ0B6ip_3OyRLysHcnhEIPQ6aKl
...
print(" and their average is",sum/count)
...
n=int(input())
count=0
st=1               #1 will be the divisor for all number, that's hy e start with 1
while st<=n:       #loop will be continue untill st become the number
    if n%st==0:    #if you divide the number with st and the reminder will 0, then you can say the number is divisor by st
        print(st)
        count+=1
    st+=1
...
M=int(input("Enter the number of height: "))
N=int(input("Enter the number of lenght: "))
...
#we solve this problem from right to left digit.
n=int(input())
i=0 #value of the power
sum=0
while n!=0:
    rem=n%10 #for the right most digit
    x=(2**i)*rem
    sum+=x
    n=n//10
    i+=1 #increase the power
print(sum)
...
while num <= end:
    ...
    # Update the counters based on whether the current number is prime or perfect
    if is_prime:
        prime_count += 1
    if is_perfect:
        perfect_count += 1
...
print("There are", prime_count, "prime numbers between", start, "and", end)
print("There are", perfect_count, "perfect numbers between", start, "and", end)
